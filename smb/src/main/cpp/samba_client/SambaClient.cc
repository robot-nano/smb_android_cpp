...
int SambaClient::Stat(const char *url, struct stat *st) {
...
int SambaClient::Rename(const char *url, const char *nurl) {
    ...
    int result = smbc_rename(url, nullptr);
    if (result < 0) {
        int err = errno;
...
int SambaClient::Unlink(const char *url) {
    ...
    if (result < 0) {
        int err = errno;
        LOGE(TAG, "Failed to unlink %s. Errno: %s.", url, err);
        return -err;
    }
...
off_t SambaClient::SeekFile(const int fd, const off_t offset, const int whence) {
...
ssize_t SambaClient::ReadFile(const int fd, void *buffer, const size_t maxlen) {
...
ssize_t SambaClient::WriteFile(const int fd, void *buffer, const size_t length) {
